...
func _ready() -> void:
	...
	# Startposition für die Wellenbewegung merken
...
func _process(delta: float) -> void:
	...
	if sonar_destroyed:
		...
		if has_node("Propeller2"):
			$Propeller2.show()
			time_passed += delta
			var scale_y = sin(time_passed * abs(propeller_speed))
...
func _physics_process(delta: float) -> void:	
	...
	if collision:
		...
		if !hit_object.is_in_group("haie") && !hit_object.is_in_group("taurus") && !hit_object.is_in_group("walOffset") && !hit_object.is_in_group("torpedo"):
			queue_free()
